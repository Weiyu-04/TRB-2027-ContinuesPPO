...
#   🔴 2026-07-27 从 1e-9 上调到 1e-6（`03` L227·**实测定的、不是拍的**）：
#   在真存档上把箱内相邻对的 |Δ| 按量级摊开（热启动 s0·616 对），分布是**双峰、中间一大段空白**：
#     · 油门：=0 占 0.2% · **(0,1e-9] 占 87.6%**（密集堆在 1e-12~1e-10）· (1e-9,1e-8] 0.5%
#            · **(1e-8, 1e-4) 完全没有点** · 之后 (1e-4,1e-3] 0.2% · >1e-3 占 11.5%
#     · 转向：=0 占 0.8% · **(0,1e-9] 占 37.7%** · **(1e-8,1e-6) 没有点** · >1e-3 占 58.9%
#   ⟹ 1e-9 这条线**落在噪声簇的尾巴里**（还留着 0.5% 的噪声没剔），而 1e-6 **落在那段空白正中**
#      ⟹ 阈值放在空白里，往左往右挪都不改结论 = 不可能被"你这个阈值是凑出来的"打。
#   物理上也站得住：油门差 1e-6 m/s² 作用 10s ⟹ 速度变 1e-5 m/s；转速差 1e-6 rad/s ⟹ 10s 转 0.0006°。都是无意义量。
#   ⟹ 与 `_BOX_TOL` / `evaluate._control_quality` 的 tol **统一成同一个数**（此前三处各写一份：1e-9 / 1e-12 / 1e-6）。
_GRID_TOL = 1e-6
