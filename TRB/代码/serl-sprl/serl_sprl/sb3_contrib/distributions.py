...
class StateDependentNoiseDistribution(Distribution):
    ...
    def proba_distribution_net(
        ...
        if self.squash_mean:
            mean_actions_net = nn.Sequential(nn.Linear(latent_dim, self.action_dim), nn.Tanh())
        else:
    ...
    def proba_distribution(
        ...
        # CRITICAL FIX: Clip latent_sde to prevent explosive variance
        # The variance is computed as latent_sde² @ std², so large latent values
        # cause numerical instability even with bounded log_std
        self._latent_sde = th.clamp(self._latent_sde, min=-5.0, max=5.0)
