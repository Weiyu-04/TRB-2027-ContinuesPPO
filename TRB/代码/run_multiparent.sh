...
_cdup=$(printf '%s\n' "${PARENTS_CKPTS[@]}" | sort | uniq -d)
[ -z "$_cdup" ] || { echo "❌ PARENTS_CKPTS 有重复 ckpt：$_cdup（同一个父当成两个=毁掉嵌套设计）"; exit 1; }
...
for i in "${!PARENTS_CKPTS[@]}"; do
  ...
  # 🔴 显式列三文件+各自 .tmp（别用 "$FROZEN".* 通配·它匹配不到下划线的 _vecnorm.pkl·复审B MEDIUM）
  chmod +w "$FROZEN.zip" "${FROZEN}_vecnorm.pkl" "${FROZEN}.progress.json" 2>/dev/null || true
  rm -f "$FROZEN.zip" "${FROZEN}_vecnorm.pkl" "${FROZEN}.progress.json" \
        "$FROZEN.zip.tmp" "${FROZEN}_vecnorm.pkl.tmp" "${FROZEN}.progress.json.tmp" 2>/dev/null || true
  ...
  chmod -w "$FROZEN.zip" "${FROZEN}_vecnorm.pkl" "${FROZEN}.progress.json" 2>/dev/null || true   # 只读=挡非 root 误覆盖（显式列三文件·root 无视权限位·非硬保证）
