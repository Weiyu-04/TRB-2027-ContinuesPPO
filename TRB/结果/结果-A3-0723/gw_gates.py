#!/usr/bin/env python3
"""只让路态(ρ2/3/4·860)门判定·进度flush。门1(前向不变正交三查)+门2(合规backup)+A成员。本机RK4·官方--gates权威。"""
...
print(f"让路态 n={len(gw)} (head-on {sum(1 for r in gw if r['rho']==2)}/crossing {sum(1 for r in gw if r['rho']==3)}/overtake {sum(1 for r in gw if r['rho']==4)})", flush=True)
st = {}
for i, r in enumerate(gw):
    ...
    d["g1a"] += int(c1a["certified_perm"]); d["g1b"] += int(c1b["certified_perm"]); d["g1c"] += int(rr(list(tjT[-1]), oT) > 0)
    if cs(g) != 0:
        d["cbt"] += 1; d["cb"] += int(fb(e, o, l, w, cs(g)) is not None)
    if (i+1) % 100 == 0: print(f"...{i+1}/{len(gw)}", flush=True)
...
for rho in sorted(st):
    ...
    print(f"  {NM[rho]:9s} {d['n']:4d}  {100*d['inA']/d['n']:5.1f}  {100*d['g1a']/nA:.0f}/{100*d['g1b']/nA:.0f}/{100*d['g1c']/nA:.0f}       {cb}", flush=True)
