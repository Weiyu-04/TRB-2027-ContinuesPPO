...
control sequence $m$ a \emph{certified straight-tail escape} for state $s$ if (i) it is
\emph{executable} by the decision-step controller---each segment boundary lands on a
decision-step ($\Delta=10$\,s) boundary, so $m$ is a sequence of constant per-step
controls; (ii) its tail segment is \emph{constant velocity and straight}
($\omega=0\wedge a=0$); and (iii) the sound Lipschitz clearance certificate (the same
interval lower bound used for Prop.~\ref{prop:farfield},
Section~\ref{sec:provable-farfield}) shows hull distance $d>0$ on $[0,H]$ with a
past-closest-point-of-approach (CPA) margin. Define the \emph{clearable set}
$A:=\{s:\exists\,\text{certified straight-tail escape}\}$. Executability (i) is
essential: a witness that switches control mid-step is not a real fallback, so the
maneuver family uses turn durations that are integer multiples of $\Delta$;
empirically this costs nothing (the clearing turns large hulls need already exceed one
step).
...
maneuver (the backup-maneuver terminal set $U_{\mathrm{term}}$), makes the shielded
closed loop \emph{recursively feasible on $A$} under a single CV target: a sound,
certified collision-free escape maneuver provably remains available at every give-way
step. We state this as recursive feasibility, \emph{not} as a collision-free guarantee
for the \emph{executed} closed loop---the latter would additionally require the
near-field per-step projection constraint (a first-order linearization, sound only in
the far-field regime of Prop.~\ref{prop:farfield}), and the executed policy's collision
freedom is reported \emph{empirically} in Section~\ref{sec:provable-summary}.
\end{proposition}
...
\begin{scope}
[HONESTY] (0) The terminal constraint certifies $A$-membership only at
\emph{give-way} successors ($\rho'\in\{$head-on, crossing, overtake$\}$); a
successor that de-escalates to no-conflict is trivially safe, while stand-on and
emergency successors fall to the existing per-step guarantees
(Props.~\ref{prop:farfield}--\ref{prop:compliance}) and the empirical emergency
fallback respectively. The forward-invariance guarantee is thus over the give-way
portion of the trajectory on $A$; we do not claim cert-level coverage of the
stand-on/emergency regimes. (i) Single CV target (multi-target is weaker,
$A=\bigcap_i A_i$, and per-target funnels may conflict). (ii) Membership in $A$ is
decided by a finite constant-control maneuver family plus the sound-but-conservative
certificate, so $A$ is an \emph{inner} approximation of the true clearable set; the residual is exactly
...
certificate-defined clearable set (Prop.~\ref{prop:forward-invariant}): on that set,
under a single CV target, a sound certified collision-free escape provably remains
available at every give-way step (\emph{recursive feasibility}), a guarantee reactive
CBF-QP filters lack under a $10$\,s hold; the executed policy's collision freedom itself
is reported empirically, not claimed as proven. We do \textbf{not} claim
...
\emph{medium} (rising to medium-strong only after the [PENDING] closed-loop
$U_{\mathrm{term}}$ wiring and official-gate reconfirmation): the invariance upgrade and
gap\#1 are sound, but coverage is
