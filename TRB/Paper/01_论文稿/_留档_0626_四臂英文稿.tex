...
sampled clearable state (official-integrator reconfirmation [PENDING]). We deliberately
do \emph{not} claim bare provable collision freedom:
...
zero hull collisions over $10$ seeds $\times$ $40$ episodes on the current benchmark
(the headline balanced/overtaking-dataset run is [PENDING]) and a structural
$\sim\!5\times$ advantage in \emph{throttle}-increment smoothness over the discrete
baseline, at comparable arrival---though yaw-increment and jerk currently trail discrete
by $\sim\!1.2$--$1.3\times$ (yaw-axis smoothing is open work). We report all limitations
honestly, including a
