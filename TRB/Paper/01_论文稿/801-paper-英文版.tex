...
The first group is the bearing sector. The bearing of the target vessel is placed in a
forward, port, starboard, or aft sector, along the sidelight boundaries of the convention.
The forward sector has a half-angle of $5^\circ$. The outer boundary of the side sectors is
$112.5^\circ$. The second group is the heading relation. It separates nearly anti-parallel
headings, nearly parallel headings, and a target vessel that heads to port or to starboard.
The overtaking band has a half-angle of $67.5^\circ$. The third group is collision
possibility. The speed set of the own vessel must intersect the collision cone of the target
vessel. The relative speed must also close the current center distance within the check
horizon. The cone has its apex at the own vessel and is tangent to the inflated occupancy of
the target vessel. It is then translated along the velocity of the target vessel. The
inflation radius is three times the length of the target vessel, the half-width of the speed
set is $1$~m/s, and the check horizon is $420$~s.
...
\label{eq:situations}
\end{equation}
and every other case falls into $\rho_0$. Overtaking and being overtaken both swap the two
vessel arguments, because overtaking places the own vessel in the aft sector of the target
vessel. The four predicates can hold at the same time in geometry. They are therefore
applied in the order written above.

Transitions are triggered by a persistence condition. Entry from the conflict-free situation
into a give-way situation has two parts. A give-way predicate must not hold now. It must
also hold at every decision step of a $60$~s reaction window under constant-velocity
extrapolation. The first part separates ``about to hold'' from ``already holds''. It also
creates one asymmetry, which Section~\ref{sec:sym} treats. The emergency situation is
...
Stand-on requires the course and the speed to be held. Each of these sets is a half-plane or
an interval on the plane of control commands. That shape is what the rest of this paper
builds on. A set of this shape can be entered by projection, so the commands do not have to
be enumerated.
...
formalization of the maritime rules \citl{krasowski2024}{Krasowski and Althoff 2024}. This
paper adds three things. The inflation radius and the half-width of the speed set are stated
explicitly. The entry into a give-way situation is made symmetric
(Section~\ref{sec:sym}). The predicate set is then connected to a constraint set on
continuous commands.
...
next step. Written directly, that condition is not convex. We therefore use a conservative
convex inner approximation. The construction has three parts.

First, the occupancy of the target vessel after one step is over-approximated by a disk.
Its center follows a constant-velocity extrapolation. Its radius already contains a safety
margin. The own vessel is over-approximated by its circumscribed circle. A sufficient
condition for the two hulls to stay apart is then simple. The distance between the two
centers must be at least the sum of the two radii, written $d_{\mathrm{safe}}$.

Second, let $n$ be the unit vector from the target vessel to the nominal end point of the
own vessel, and let $\delta$ be that distance. The nominal end point follows from the
policy command after clipping to the action box.
...
margin the constraint acts at a center distance of about $590$--$840$~m. The median over
this scenario library is about $714$~m. Hull contact needs a much smaller distance. The
margin thus absorbs the linearization residual. The cost is a tighter constraint. Some
commands that are in fact safe are rejected.

The linearization in Equation~\eqref{eq:ucf} holds only locally. In the far field a
sufficient condition is available that does not rely on it.
...
The feasible set is an intersection of half-planes. It can be written as
$U_{\mathrm{safe}}=\{u:Au\le b\}$. The first four rows are the action box. The fifth row is
the compliant-direction half-plane. The sixth row is the collision-free constraint of
Equation~\eqref{eq:ucf}. This is a quadratic program in two variables with at most six
constraints. It is small enough to solve online at every decision step.

This construction gives the central guarantee of the paper.
