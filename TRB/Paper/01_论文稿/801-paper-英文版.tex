...
\begin{table}[htbp]
 ...
 \textbf{Notes.} A check mark is on, a cross is off, and a dash is not applicable. Rows are
 ordered along the ladder, and each row differs from its neighbor by one variable. Three
 steps carry a companion variable that cannot be separated. The action space changes
 together with the policy distribution, the continuous-only shaping is a bundle of four
 items, and the bounded distribution also changes the initial exploration scale. Those
 three steps can only be attributed as a whole.
 \end{minipage}
\end{table}
%------------------------------------------------------------------------------
...
\begin{table}[htbp]
 ...
 \begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}lcccccccc@{}}
  ...
  Ablation, bounded     & \prov{8} & \prov{7/8} & \prov{86.0 [50,\,91]} & \prov{\textbf{0.00}} & \prov{0.52} & \prov{0.0087} & \prov{0.0176} & \prov{1.97} \\
  Ablation, symmetric   & \prov{8} & \prov{5/8} & \prov{75.5 [0,\,82]} & \prov{\textbf{0.00}} & \prov{0.77} & \prov{0.0150} & \prov{0.0143} & \prov{3.02} \\
...
\begin{figure}[htbp]
 ...
 bootstrap $95\%$ intervals resampled over seeds. The lower right of panel (a) is the ideal
 region. The bars are wide because every configuration contains a diverged seed.}
 \label{fig:tradeoff}
\end{figure}
...
\begin{figure}[htbp]
 ...
 pulls the band to zero. Panel (d) counts all eight seeds. The ablation configurations are
...
\begin{figure}[htbp]
 ...
 \includegraphics[height=4.4in,keepaspectratio]{figs/fig5_shield_behaviour.pdf}
