...
\begin{table}[htbp]
 ...
 from the discrete baseline to the full method (Ours). Three steps of the ladder carry a companion
...
%------------------------------------------------------------------------------
\begin{table}[htbp]
 \caption{Main comparison, all seeds. \capnote{\textbf{Values are provisional and come
 from the validation set. The table is replaced after the re-evaluation on the test
 set.}}}
 \label{tab:main}
 \centering\small
 ...
 \begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}lcccccccc@{}}
  ...
  Ours                  & \prov{8} & \prov{\textbf{8/8}} & \prov{88.5 [81,\,92]} & \prov{\textbf{0.00}} & \prov{\textbf{0.49}} & \prov{0.0085} & \prov{0.0193} & \prov{2.56} \\
  \bottomrule
 \end{tabular*}
...
\begin{figure}[htbp]
 ...
 \caption{Safety and efficiency. \capnote{Points are medians across seeds and error bars
 are bootstrap $95\%$ intervals resampled over seeds. Circles are continuous action spaces,
...
\begin{figure}[htbp]
 ...
 and error bars are bootstrap $95\%$ intervals. Above each bar are the change relative to
 the leftmost bar, the paired signed-rank result, and the number of seeds that moved in the
 same direction. The arrival rate is bimodal, so it is drawn as a box plot.}}
 \label{fig:ablation}
\end{figure}
...
\prov{$2.4$} violations per episode. The shielded configurations sit at a collision rate of
zero and near \prov{$0.5$} violations per episode. The shield removes risk from the outcome,
...
convergence rule declared in advance, all \prov{$8$} seeds of Ours reach a
...
reaches \prov{$6$} (Figure~\ref{fig:curves}e). The two changes therefore improve more than
