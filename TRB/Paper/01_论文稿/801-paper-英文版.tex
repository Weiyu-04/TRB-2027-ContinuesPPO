...
Three levels are an exception, because a companion variable switches with the one under
study. The move to a continuous action space also changes the policy distribution. The
continuous-only shaping is a bundle of four items. The bounded action distribution also
raises the initial exploration scale, by about a factor of $2.5$ on the acceleration axis.
These three levels are attributed as a whole.
...
\begin{table}[htbp]
 ...
 \begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}l ccccccc@{}}
  ...
  Bounded only          & $\checkmark$ & $\checkmark$ & $\times$ & $\times$ & $\checkmark$ & $\checkmark$ & $\times$ \\
  Symmetric only        & $\checkmark$ & $\checkmark$ & $\times$ & $\times$ & $\checkmark$ & $\times$ & $\checkmark$ \\
  \bottomrule
 \end{tabular*}

 \vspace{4pt}
 \footnotesize
 \textbf{Notes.} $\checkmark$ on, $\times$ off, -- not applicable.
\end{table}
%------------------------------------------------------------------------------
...
attribution would fail. The unbounded Gaussian is also the form that pushes many samples
past the action box. A small re-evaluation was run during the exploration phase, and that
setting is not the one used in the formal experiment. About $73\%$ of the decision steps of
that policy sat against the boundary of the action box. The gain attributed to the shield is
therefore measured against the weakest form of continuous action. For the same reason the
step from the discrete baseline to the unshielded continuous configuration can be negative,
and it should be read as a conservative lower bound.
...
\noindent\textbf{Training budget.}\ Each configuration is trained with $8$ random seeds and
one run per seed. The $72$ runs consume $730{,}873{,}856$ environment steps in total, or
about $10.2$ million per run. The scenario split is fixed
...
part. Each run leaves $20$ checkpoints, each of which is evaluated once on the validation
set. The criterion is the highest arrival rate on that set, ties go to the earlier
checkpoint, and the test set is never touched.
...
same-machine rule was written to prevent. The best-checkpoint version and the last-checkpoint
...
%------------------------------------------------------------------------------
\begin{table}[htbp]
 \caption{Main comparison, all seeds. \textbf{Provisional: last-checkpoint values on the
 validation set.}}
 \label{tab:main}
 \centering\footnotesize
 ...
 \begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}lcccccccc@{}}
  ...
  Bounded only          & \prov{8} & \prov{7/8} & \prov{86.0 [50,\,91]} & \prov{\textbf{0.00}} & \prov{0.52} & \prov{0.0087} & \prov{0.0176} & \prov{1.97} \\
  Symmetric only        & \prov{8} & \prov{5/8} & \prov{75.5 [0,\,82]} & \prov{\textbf{0.00}} & \prov{0.77} & \prov{0.0150} & \prov{0.0143} & \prov{3.02} \\
  \bottomrule
 \end{tabular*}

 \vspace{4pt}\footnotesize
 \textbf{Notes.} The best value in each column is in bold. The turn and throttle increments
 are not marked, because they cannot be compared across action spaces
 (Section~\ref{sec:train}). The share of emergency steps is descriptive.
\end{table}
%------------------------------------------------------------------------------
...
\begin{figure}[htbp]
 ...
 \caption{Safety and efficiency. (a) Arrival rate against COLREGs violations per episode, on
 a logarithmic axis. Points are medians across seeds and bars are bootstrap $95\%$ intervals
 resampled over seeds. Circles are continuous action spaces, triangles are discrete ones,
 and filled markers carry a shield. (b) Collision rate, one point per seed, with a bar at
 the median. \textbf{Provisional: values are from the validation set.}}
 \label{fig:tradeoff}
\end{figure}
...
the discrete configurations do not have. The bounded action distribution carries a companion
variable, namely the initial exploration scale, as stated in
Section~\ref{sec:setup}. A converged-seeds-only version is given for this group
as well.

\begin{figure}[htbp]
 \centering
 \includegraphics[width=\linewidth]{figs/fig3_ablation.pdf}
 \caption{The $2\times2$ ablation, paired by seed. (a) and (b) Bars are medians across seeds
 and error bars are bootstrap $95\%$ intervals resampled over seeds. Above each bar are the
 change relative to the leftmost bar, the mark of the paired signed-rank test, and the
 number of seeds that moved in the same direction. (c) Box plot of the arrival rate, with
 the number of diverged seeds below each box and open points for single seeds.
 \textbf{Provisional: values are from the validation set.}}
 \label{fig:ablation}
\end{figure}
...
\begin{figure}[htbp]
 ...
 \caption{Sample efficiency and training reliability against training steps. Thick lines are
 medians and shaded bands are interquartile ranges, both over converged seeds only. The
 legend gives the number of converged seeds out of $8$. The axis starts at the first
 evaluation point. (a) Arrival rate. (b) Collision rate. (c) COLREGs violations per episode.
 (d) Seeds that meet the convergence rule, counted over all $8$. (e) Share of steps under
 emergency control. (f) Explained variance of the value function. \textbf{Provisional:
 values are from the validation set.}}
 \label{fig:curves}
\end{figure}
...
\begin{figure}[htbp]
 ...
 \caption{Behavior of the shield during training. (a) Which branch produced each control
 command, on a logarithmic axis. (b) Saturation of the action, with the turn axis solid and
 the acceleration axis dashed. (c) Distribution of the size of the projection correction,
 with single samples on the left half, a kernel density on the right half, and the median as
 a black line. (d) Share of each encounter situation, on a logarithmic radius.
 \textbf{The angle is the sector in which the convention defines the situation, not a
 measured bearing distribution.} The emergency situation belongs to no sector and is drawn
 as a dashed ring. \textbf{Values are collected during training.}}
 \label{fig:shield}
\end{figure}
...
symmetric-only configuration, at \prov{$0.093$}, and smallest for the
bounded-only configuration, at \prov{$0.066$}. That ordering matches the turn
...
\begin{table}[htbp]
 ...
 \begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}llccccccc@{}}
  ...
  \addlinespace
  This paper & Policy range & \multicolumn{7}{c}{(see Table~\ref{tab:main})} \\
  \bottomrule
 \end{tabular*}

 \vspace{4pt}\footnotesize
 \textbf{Notes.} Each baseline is reported at its own best setting, tuned on a set that is
 disjoint from the one used here. The policy range is $\pm0.048$ and $\pm0.018$, the same
 authority as the policy of this paper, and the actuator limits are $\pm0.24$ and
 $\pm0.03$. Both are reported so that a difference in authority is not a confounder.
 \textbf{The arrival rate is not a measure of collision avoidance.} These controllers have
 no terminal alignment stage. They reach the goal position in $96.8\%$ to $99.7\%$ of
 episodes and then fail the terminal heading gate.
\end{table}
%------------------------------------------------------------------------------
