...
\abshead{Methods.}We write the give-way clause as a half-plane constraint on the plane of control commands. We intersect it with the actuator limits and a one-step collision-free condition, which gives a convex safe set. A small quadratic program projects the policy command onto the nearest point of that set. The projection sits inside the environment transition, so the policy learns under its own corrected commands. The data are 2,000 simulated two-ship encounters, split into 1,400 training and 600 test cases. Nine configurations form a single-variable ladder, each trained with eight fixed seeds.

\abshead{Findings.}On every give-way step where the projection is feasible, directional compliance holds by construction. It does not depend on the training outcome or the random seed. Steps that switch to the fallback branch are not covered. On the test set, the violation count per episode is \TBD\ and the collision rate is \TBD. The ablation gains are significant under a paired signed-rank test. We do not claim the arrival rate as a benefit.

\abshead{Novelty.}To our knowledge, this is the first projection-based safety mechanism that enforces directional COLREGs compliance on continuous control commands. We also give a graded statement of what is proved and of what is not.
