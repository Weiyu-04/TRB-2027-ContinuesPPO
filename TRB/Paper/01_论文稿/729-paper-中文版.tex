...
\textbf{建模对象与占据表示。}\ 我们以一条 $L=175$~m $\times$ $W=25.4$~m 的中型集装箱船为对象，其操纵性能界为 $a_{\max}=0.24~\mathrm{m/s^2}$、$\omega_{\max}=0.03~\mathrm{rad/s}$、$v_{\max}=9.5~\mathrm{m/s}$，决策周期 $\Delta t=10$~s。这一组数值刻画的是大型商船的典型量级：转艏率上限折合仅约 $1.7^\circ/\mathrm{s}$，意味着一次 $20^\circ$ 的明显转向需要十余秒才能完成，\emph{慢于一个决策周期}。这一"操纵慢于决策"的特征贯穿全文——它既是安全约束必须前瞻的原因，也是 “\nameref{sec:guarantee}”一节 中递归可行性得以成立的几何基础。

船体占据用矩形表示。我们同时使用它的两个圆近似，各有明确分工：
\begin{equation}
  R_{\mathrm{circ}}=\tfrac12\sqrt{L^2+W^2}=88.4~\mathrm{m},
  \qquad
  r_{\mathrm{insc}}=\tfrac12 W=12.7~\mathrm{m},
  \label{eq:hull}
\end{equation}
其中外接圆 $\mathrm{disk}(R_{\mathrm{circ}})$ \emph{过近似}船体，用于构造"一定不碰"的充分条件（圆盘不交 $\Rightarrow$ 船体不交）；内切圆 $\mathrm{disk}(r_{\mathrm{insc}})$ \emph{欠近似}船体，用于构造"一定会碰"的充分条件（内切圆与障碍相交 $\Rightarrow$ 船体与障碍相交）。两个方向的判据各取一个圆，是保证两侧结论都单侧保守的关键。
...
\begin{equation}
  ...
  \label{eq:reward}
\end{equation}
$r_{\mathrm{sparse}}$ 为终止事件的稀疏项（到达 $+50$；超时 $-25$、出界 $-5$、停住 $-40$、碰撞 $-50$、每一步紧急控制 $-0.5$）；$r_{\mathrm{goal}}=c_{\mathrm{reach}}\bigl(\lVert p_{t-1}-p_g\rVert-\lVert p_t-p_g\rVert\bigr)$ 为趋近目标的稠密项（$c_{\mathrm{reach}}=1.5$）；$r_{\mathrm{velocity}}$ 惩罚航速偏离巡航区间（系数 $-2.0$）；$r_{\mathrm{deviate}}$ 惩罚偏离起点—目标参考线（系数 $-0.001$）。$r_{\mathrm{colregs}}$ 是一项随距离衰减的软性合规惩罚，仅在软性奖励对照组中启用；在我们的方法中合规由式~\eqref{eq:ucolregs} 的硬约束承担，故该项置零。
